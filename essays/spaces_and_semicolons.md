...
By default, Python (and practically all other languages) implements its own coding standard. Each line in a block of code must have the same indentation. However, this is not just to make the code look prettier. The lack of curly braces in Python to denote a block of code means that the interpreter cannot understand your code if it does not implement this standard.
